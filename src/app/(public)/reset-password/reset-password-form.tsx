...
export function ResetPasswordForm({ isSetup }: { isSetup: boolean }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    router.replace(isSetup ? "/fleet" : "/login?reset=success");
  }
      ...
      <div className="w-full max-w-sm space-y-7">
        ...
        <div>
          <h2 className="text-3xl font-bold text-foreground">
            {isSetup ? "Crea tu contraseña" : "Nueva contraseña"}
          </h2>
          <p className="text-base text-foreground/80 mt-1">
            {isSetup
              ? "Elige una contraseña para acceder a tu flotilla."
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button type="submit" className="w-full" size="lg" disabled={loading}>
            {loading ? "Guardando…" : isSetup ? "Crear contraseña y entrar" : "Guardar contraseña"}
          </Button>
        </form>
      </div>
    </main>
